...
from nes.core.models.person import Gender, PersonDetails, Education, Position
from nes.core.models import Address
from datetime import date
...
async def fix_political_parties(context: MigrationContext) -> None:
    ...
    for party_data in new_parties:
        ...
        address = Address(
            description2=LangText(
                ...
                ne=LangTextValue(
                    value=party_data["address_nepali"],
                    provenance="imported"
                ),
            )
        )
        ...
        party_chief = LangText(
            en=LangTextValue(
                value=party_data["chief_english"],
                provenance="translation_service"
            ),
            ne=LangTextValue(
                value=party_data["chief"],
                provenance="imported"
            ),
        )
        ...
        symbol = PartySymbol(
            name=LangText(
                ...
                ne=LangTextValue(
                    value=party_data["symbol_name_nepali"],
                    provenance="imported"
                ),
            )
        )
        ...
        party_entity_data = {
            ...
            "attributions": [
                Attribution(
                    title=LangText(
                        en=LangTextValue(
                            value="Nepal Election Commission",
                            provenance="human"
                        ),
                        ne=LangTextValue(
                            value="नेपाल निर्वाचन आयोग",
                            provenance="human"
                        ),
                    ),
                    details=LangText(
        ...
        context.log(f"✓ Created party: {party_data['english_name']} ({party_data['slug']})")
        created_count += 1
    ...
    for fix in existing_fixes:
        ...
        current_nepali_name = party_entity.names[0].ne.full if party_entity.names[0].ne else None
        current_english_name = party_entity.names[0].en.full if party_entity.names[0].en else None
        
        # Check if corrected name already exists in alternate names
        corrected_already_exists = False
        for i, name in enumerate(party_entity.names):
            if i == 0:  # Skip primary
                continue
            if name.ne and name.ne.full == corrected_nepali_name:
                corrected_already_exists = True
                break
        ...
        if fix_type == "misspelled_name":
            ...
            updated_names = [
                Name(
                    kind=NameKind.PRIMARY,
                    en=NameParts(full=corrected_english_name) if corrected_english_name else party_entity.names[0].en,
            ...
            if len(party_entity.names) > 1:
                for name in party_entity.names[1:]:
                    ...
                    if name.ne and (name.ne.full == corrected_nepali_name or name.ne.full == current_nepali_name):
                        continue
                    updated_names.append(name)
        ...
        context.log(f"✓ Fixed party name ({slug}): {current_nepali_name} → {corrected_nepali_name}")
        updated_count += 1
...
async def import_candidates(context: MigrationContext) -> None:
    ...
    id_matches = {}
    with open(context.migration_dir / "data" / "candidate_id_matches_2079_2082.csv", "r") as f:
    ...
    district_slug_map = {}
    with open(context.migration_dir / "data" / "district-to-slug.csv", "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
    ...
    for person in all_persons:
        if person.identifiers:
            for identifier in person.identifiers:
                if identifier.name and identifier.name.en and "nec_candidate_id" in identifier.name.en.value.lower():
    ...
    for person, raw, translated, candidate_id_2082 in updated_candidates:
        ...
        context.log(f"✓ Updated: {person.names[0].ne.full if person.names[0].ne else person.names[0].en.full}")
    ...
    for person_data in person_data_list:
        ...
        context.log(f"✓ Created: {person.names[0].ne.full if person.names[0].ne else person.names[0].en.full}")
...
def _build_person_data(
    ...
    if inst_en or inst_ne or degree_en or field_en:
        education = [
            Education(
                institution=(
                    LangText(
                        en=LangTextValue(value=inst_en, provenance="translation_service") if inst_en else None,
                        ne=LangTextValue(value=inst_ne, provenance="imported") if inst_ne else None,
                    )
    ...
    personal_details = PersonDetails(
        ...
        address=Address(
            description2=LangText(
                en=LangTextValue(value=addr_en, provenance="translation_service") if addr_en else None,
                ne=LangTextValue(value=addr_ne, provenance="imported") if addr_ne else None,
            )
        ) if addr_en or addr_ne else None,
        education=education,
        positions=positions,
    )
        ...
        "election_council_misc": {
            ...
            "qualification": (
                LangText(
                    ...
                    ne=LangTextValue(value=qual_ne, provenance="imported") if qual_ne else None,
                ).model_dump()
            ...
            "other_details": (
                LangText(
                    en=LangTextValue(value=other_en, provenance="translation_service") if other_en else None,
                    ne=LangTextValue(value=other_ne, provenance="imported") if other_ne else None,
                ).model_dump()
    ...
    return {
        ...
        "names": [
            Name(
                ...
                ne=NameParts(full=name_extractor.standardize_name(raw["CandidateName"])),
            ).model_dump()
        ],
